...
\usepackage{booktabs}
%%%%%%%%%%%%%%%%%%%% Bibliography Setup
% Load gói biblatex với các tùy chọn:
% backend=biber: Trình xử lý biên dịch (mạnh hơn BibTeX cũ)
% natbib=true: Cho phép dùng các lệnh của natbib như \cite, \citep
% style=alphabetic: Kiểu trích dẫn chữ cái (ví dụ: [New24])
% maxbibnames=50: Hiển thị tối đa 50 tên tác giả trước khi dùng et al.
\usepackage[backend=biber,natbib=true,style=alphabetic,maxbibnames=50]{biblatex}
\addbibresource{D:/GitHub/STAC-UMT/references.bib} 
%%%%%%%%%%%%%%%%%%%%
\usepackage{tabto}
\usepackage{caption}[=v3.0]
\usepackage[colorlinks,linkcolor=red,filecolor=magenta,citecolor=green,urlcolor=STACBlue,hyperindex,plainpages=false,unicode]{hyperref}
...
\definecolor{STACBlue}{RGB}{28, 166, 222}
...
\fancypagestyle{solutionpage}{ % Trang lời giải
  ...
  \renewcommand{\footrulewidth}{0pt}
  \fancyfoot{}
  \fancyfoot[L]{\textcolor{black}{\textbf{\href{https://www.facebook.com/STAC.UMT/}{CÂU LẠC BỘ HỌC THUẬT KHOA CÔNG NGHỆ UMT}}} \\ \textcolor{black}{\text{UMT School of Technology Academic Club}}}
  \fancyfoot[R]{
    \textcolor{black}{\textbf{Trang {\thepage}}}
  }
  \fancyhead{}
  \fancyhead[R]{
    \InsertQR
...
%%%%%%%%%%%%%%%%%%%% Karnaugh (Like Rosen's book) https://tex.stackexchange.com/questions/527500/k-map-diagrams-from-rosen
\makeatletter
...
\tikzset{k-map diagram/.style={matrix of math nodes,nodes in empty cells,
    nodes={draw,minimum size=3.6em,anchor=center},
    column 1/.style={nodes={draw=none,minimum size=0em,text=UMTBlue,font=\normalsize}},
    row 1/.style={nodes={draw=none,minimum size=0em,text=UMTBlue,font=\normalsize}},
    column sep=-2*\pgflinewidth,row sep=-1*\pgflinewidth,
    append after command={let 
     ...
     \ifcase\mydim
      \draw[UMTBlue] #1 circle[radius=3ex];
     \or
      \def\mysplit##1,##2;{\edef\myA{##1}\edef\myB{##2}}
      \mysplit#1;
      \draw[UMTBlue] let \p1=\myA,\p2=\myB,\n1={atan2(\y2-\y1,\x2-\x1)}
       in 
       ($ (\p1)!3ex!90:(\p2) $) 
       arc[start angle=\n1+90,end angle=\n1+270,radius=3ex] --
        ($ (\p2)!3ex!90:(\p1) $) 
       arc[start angle=\n1-90,end angle=\n1+90,radius=3ex] -- cycle;
     \fi
    }},
    pics/cfit/.style={code={
     \pgfmathtruncatemacro{\mydim}{Dim("#1")-1}
     \ifcase\mydim
      \draw[UMTBlue] ($#1+(0,3ex)$)
        arc[start angle=90,end angle=-90,radius=3ex]
        -- ++ (-\pgfkeysvalueof{/tikz/cfit/arm},0) -- ++ (0,6ex) -- cycle;
      \or
       \def\mysplit##1,##2;{\edef\myA{##1}\edef\myB{##2}}
      \mysplit#1;
...
\NewDocumentCommand{\LayCauVaDiem}{m O{JetBlack}}{%
    {
        ...
        \ifcsdef{diemso@#1}{%
            % Lấy giá trị thực của điểm đưa vào biến tạm \diemTam
            \letcs{\diemTam}{diemso@#1}%
            % Kiểm tra: Nếu \diemTam KHÔNG rỗng thì mới in
            \ifdefempty{\diemTam}{}{%
                 \ \textcolor{UMTRed}{\textbf{(\diemTam\ điểm)}}%
            }%
        }{}%
...
% LỆNH IN: HỘP ĐỀ BÀI TRONG TCOLORBOX (CÂU X + NỘI DUNG)
% Cấu trúc: Khung tcolorbox (nền cyan nhạt, viền đen nhạt) bao quanh Câu hỏi.
%
% Cú pháp: \LayDeBaiBox{Mã_Câu_Hỏi}[Màu_Của_Chữ_Câu]
% Ví dụ:   \LayDeBaiBox{bai:trr-1}[UMTBlue] -> Chữ "Câu X" màu UMTBlue, Nội dung giữ màu gốc.
%          \LayDeBaiBox{bai:trr-1}          -> Tất cả chuyển sang hệ màu đen (Chế độ in ấn).
%          \LayDeBaiBox{bai:trr-1}[black]   -> Tất cả chuyển sang hệ màu đen (Giống mặc định).
\NewDocumentCommand{\LayDeBaiBox}{m O{black}}{%
    \begin{tcolorbox}[
        colback=cyan!10!white,
        opacityback=0,
        enhanced jigsaw,
        ...
        % Logic điều chỉnh màu thông minh
        \def\tempparam{#2}
        \ifblank{#2}{\ChuyenHeMauDen}{}%
        \ifdefstring{\tempparam}{black}{\ChuyenHeMauDen}{}%
        ...
        \ifblank{#2}
            {\noindent \textbf{\textcolor{black}{Câu \thesocauhoi.}}}
            {\noindent \textbf{\textcolor{#2}{Câu \thesocauhoi.}}}%
        \useproblem[ds]{#1}%
    \end{tcolorbox}
}
%%%%%%%%%%%%%%%%%%%%
...
\usepackage{tasks}
\settasks{
    label-offset = 0.75em, % Khoảng cách giữa nhãn (1) và chữ
    label-width = 1em % Độ rộng dành cho nhãn
}
%%%%%%%%%%%%%%%%%%%%
...
\newcommand{\InsertQR}{%
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south east, 
              xshift=-0.3cm,       % Canh lề (ngang) 
              yshift=-2.1cm]        % Canh lề (dọc) 
              at (current page.north east) 
        {
            % Đảm bảo đường dẫn ảnh đúng
            \includegraphics[height=1.5cm]{Resources/Images/STAC_QR.png} 
        };
    \end{tikzpicture}%
}
%%%%%%%%%%%%%%%%%%%%